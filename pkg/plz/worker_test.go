...
func Test_complete_correctDefinitionOfTestRun(t *testing.T) {
	...
	var (
		...
		defaultTestRun = v1alpha1.TestRun{
			...
			Spec: v1alpha1.TestRunSpec{
				...
				Parallelism: int32(0),
				Separate:    false,
				Arguments:   `--no-thresholds --log-output=loki=https://cloudlogs.k6.io/api/v1/push,label.lz=,label.test_run_id=0,header.Authorization="Token $(K6_CLOUD_TOKEN)" --include-system-env-vars=false`,
	...
	cloudFieldsTestRun.Spec.Arguments = strings.Replace(cloudFieldsTestRun.Spec.Arguments,
		`--no-thresholds`,
		`--tag load_zone=some-zone --no-thresholds`,
		1)
